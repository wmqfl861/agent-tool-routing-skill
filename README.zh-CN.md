...
> 当前版本：**v0.1.0**。项目仍处于 1.0 之前；应用到共享或生产 Agent 环境前，
> 请先审阅版本差异。
...
在仓库根目录为所有支持的 Agent 安装架构 Skill 和 onboarding 规则：
...
CI 会在 Windows、Ubuntu、macOS 上运行仓库验证，并使用 Windows PowerShell
5.1 及各平台 PowerShell 7 执行安装器测试。
