...
> 当前版本：**v0.1.3**。项目仍处于 1.0 之前；应用到共享或生产 Agent 环境前，
> 请先审阅版本差异。
...
- Linux/macOS：PowerShell 7.2 或更高版本（`pwsh`）。
- 仅运行仓库 validator 时需要 Python 3 和 PyYAML。
- 仅运行安装器测试时需要 Pester 5.7.1。

根据操作系统选择命令。以下步骤会克隆仓库，并为所有受支持的 Agent 安装架构
Skill 和 onboarding 规则。
...
使用 Windows PowerShell 5.1 执行安装器：
...
安装 PowerShell 7.2 或更高版本后，在 Bash 中执行：
...
安装 PowerShell 7.2 或更高版本后，在 zsh 中执行：
...
如果使用下载的仓库压缩包，请进入解压后的仓库根目录，只执行对应平台的最后一条
安装器命令。

以下示例均在仓库根目录使用 `pwsh`，适用于 Windows 上的 PowerShell 7、Linux
和 macOS。使用 Windows PowerShell 5.1 时，将 `pwsh` 替换为 `powershell.exe`。
...
| 平台 | 运行时 | 路径策略 | CI 覆盖 |
| --- | --- | --- | --- |
| Windows | Windows PowerShell 5.1、PowerShell 7 | 仅本地磁盘；拒绝 UNC、device namespace、网络路径和不安全 reparse point。 | 两套 shell 的 Pester 和仓库验证。 |
...
├── scripts/install.ps1      # 跨平台事务式安装器
├── scripts/validate-skill.py # 仓库契约 validator
