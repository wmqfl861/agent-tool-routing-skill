...
> 当前版本：**v0.2.3**。项目仍处于 1.0 之前；应用到共享或生产 Agent 环境前，
> 请先审阅版本差异。
...
`11,916` metadata bytes 和 `98,901` body bytes，合计 `110,817` bytes。受支持
路径的总负载分别为：strict-progressive A `7,287`、strict-progressive B
`4,779`、auto-discovery A `2,783`、auto-discovery B `2,375`、C bypass `0`。
...
| 可选工具生命周期门禁 | 显式安装后，将简短删除/卸载请求视为完整受管下线，并执行安全所有权检查。 |
...
| 跨平台安装器 | 只安装显式指定的单个 Agent；仅显式使用 `-Target all` 时才同时安装三者。 |
...
使用 `-AddOnboardingRules` 安装可选门禁后，对于名称或上下文已经明确的能力，
用户只需说“删除 Example Crawler”或“卸载 Example Crawler”，门禁就会把该请求
委托给架构 Skill，并授权完整的受管下线流程。Agent 会先备份受影响状态，
...
如果没有安装这个可选门禁，直接的工具生命周期请求本身不会选中自动发现的架构
Skill。仅安装 Skill 时，必须显式调用架构 Skill 才会进入受管下线流程。
...
pending 请求默认保持静默，只有当前用户明确要求目标 Agent 初始化或恢复路由时才会
执行；它不得替代无关的当前任务。请求只对其中记录的 `target_agent`、
`target_config_root` 和单 Agent 修改范围有效。正在运行的 Agent 不保证热加载刚安装
的 Skill，因此应在目标 Agent 的全新会话中显式恢复该任务。
...
根据操作系统和 Agent 选择一条命令。每条命令只为显式目标安装经过校验的架构
Skill，不会修改其他 Agent，不会写入全局 onboarding/runtime 规则，也不会排队
索引；这些行为都需要单独的显式开关。命令可在任意目录执行，不需要 Git。

命令固定到 `v0.2.3`：先把 bootstrap 下载到私有临时文件，在执行前核对命令内置
...
快速开始命令在完成经过校验的单 Agent 安装后即停止。只有明确需要目标 Agent 的全局
生命周期门禁时才增加 `-AddOnboardingRules`；只有明确要排队索引任务时才增加
`-InitializeRouting`。pending 任务保持静默，直到当前用户明确要求请求中记录的目标
Agent 初始化或恢复路由；两个开关都不授权写入其他 Agent 的配置根目录。
...
随后在全新的 Codex 会话中明确要求“初始化或恢复路由”。pending 请求绝不能优先于
无关的当前任务。只有确实要同时修改 Codex、Claude Code 和 zcode 时才使用
`-Target all`。
...
`-AddGlobalRules` 仅保留用于返回迁移错误，因为一个开关不应静默启用两类独立的
全局规则。请显式使用 `-AddOnboardingRules` 和/或 `-AddRuntimeRules`。
远程 bootstrap 仍兼容接受 `-SkipOnboardingRules`；由于默认行为已经是仅安装
Skill，该开关现在是冗余的，并且不能与 `-AddOnboardingRules` 同时使用。
...
- 请求使用某个工具，不代表允许安装、认证、购买、发布、删除、切换 provider 或
  修改生产环境。
- 当可选 onboarding 门禁委托该请求，或用户为此工作显式激活架构 Skill 时，请求
  删除、移除或卸载一个身份明确的工具，已经授权该工具的完整受管下线，但不授权
  删除共享或人工修改的产物、凭据、缓存、用户数据、账户或其他能力。
...
```text
使用 $tool-use-architecture 审计当前 Codex 的路由架构。
```
...
`tests/test_activation_boundaries.py` 只校验 metadata、已安装规则和非干扰契约，
不会实现一个替代模型选择器，也不声称证明未来所有宿主模型的真实 Skill 自动选择
行为；宿主级结论必须来自单独留档的 runtime 运行。
